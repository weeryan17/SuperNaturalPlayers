...
public class VampBlCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (sender instanceof Player) {
            if (cmd.getName().equalsIgnoreCase("bl") && Main.dataConfig().get("Players." + playerName + ".type").toString().equals("Vampire")) {
                if (args.length == 0) {
                	if(Main.dataConfig().getBoolean("Players." + playerName + ".BL") == false){
                		Main.dataConfig().set("Players." + playerName + ".BL", true);
                    blood(playerName, p);
                    sender.sendMessage(ChatColor.RED + "You went into blood lust mode. This will drain your blood");
                } else {
                	Main.dataConfig().set("Players." + playerName + ".BL", false);
                	stop(p);
    ...
    public void blood(final String sender, final Player p) {
        double blood = Main.dataConfig().getDouble("Players." + sender + ".Blood");
        if (blood >= 0.1) {
            Main.dataConfig().set("Players." + sender + ".Blood", (blood - 0.8));
    ...
    public void blood2(String sender, Player p) {
        double blood = Main.dataConfig().getDouble("Players." + sender + ".Blood");
        if (blood >= 0.1) {
            Main.dataConfig().set("Players." + sender + ".Blood", (blood - 0.2));
